...
def load_enemies(filename):
    filename = "data/" + filename
    with open(filename, 'r') as enemyfile:
...
player_image = load_image('main9.png')
...
while True:
    ...
    if True in (player.moveup, player.movedown, player.moveleft, player.moveright):
        if player.move():
            level += 1
            sound1.stop()
            all_sprites = pygame.sprite.Group()
